...
export class NTPDefault {
  ...
  addNode(nodeType, text) {
    ...
    this._data.nodes[nodeKey] = { ID, type, text, links: [], tags: [] };
    this._data.rootNodes[nodeKey] = ID;
    return nodeKey;
  }
  ...
  getNodeRaw(nodeKey) {
    // TODO: Újragondolni a következményeit h magát az object-et visszaadom.
  ...
  addNodeLink(fromNodeKey, linkType, toNodeKey) {
    if (this.getNodeLinks(fromNodeKey, linkType, toNodeKey).length === 0) {
      this._data.nodes[fromNodeKey].links.push(
        { to: toNodeKey, type: linkType, tags: [] },
      );
      if (linkType === "parent") {
  ...
  getNodeLinks(nodeKey, linkType, toNodeKey) {
    return this._data.nodes[nodeKey].links
      .filter((link) => ((link.to === toNodeKey || !toNodeKey) && (link.type === linkType || !linkType)))
      .map((link) => ({ to: link.to, type: link.type, tags: [...link.tags] }));
  }

  deleteNodeLinks(nodeKey, linkType, toNodeKey) {
      this._data.nodes[nodeKey].links = this._data.nodes[nodeKey].links
        .filter((link) => !((link.to === toNodeKey || !toNodeKey) && (link.type === linkType || !linkType)));
  }
